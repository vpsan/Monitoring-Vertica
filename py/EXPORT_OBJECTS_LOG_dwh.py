...
dwh_schema_tablea_name_lst = ['SCHEMA_NAME_1.TABLE_1', 'SCHEMA_NAME_2.TABLE_2']

while True:
    df_list = []
    for schema_table in dwh_schema_tablea_name_lst:
        query_start_ts = datetime.now().replace(microsecond=0)
        df = read_sql_query(f'''SELECT EXPORT_OBJECTS('', '{schema_table}')''', eng_dwh)
    ...
    df_to_push.to_sql(name='EXPORT_OBJECTS_LOG', con=eng_dwh, if_exists='append', schema='SCHEMA_NAME', index=False)
    sleep(900)