...
while True:
    df = read_sql_query(f'SELECT * FROM v_monitor.resource_queues', eng_tst)
    if df.empty:
    ...
    df.to_sql(name='RESOURCE_QUEUES_LOGS', con=eng_tst, if_exists='append', schema='SCHEMA_NAME', index=False)
    sleep(300)